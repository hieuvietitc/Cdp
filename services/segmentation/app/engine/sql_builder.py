...
class SQLBuilder:
    ...
    def _event_condition(
        ...
        subquery_parts = [
            ...
            f"WHERE e.profile_id = profiles.id",
            f"AND e.event_type = '{event_type}'",
        ]
        ...
        if time_window:
            ...
            if last_n_days:
                subquery_parts.append(
                    f"AND e.occurred_at >= NOW() - INTERVAL '{int(last_n_days)} days'"
                )
            if from_date:
                subquery_parts.append(f"AND e.occurred_at >= '{from_date}'")
            if to_date:
                subquery_parts.append(f"AND e.occurred_at <= '{to_date}'")
